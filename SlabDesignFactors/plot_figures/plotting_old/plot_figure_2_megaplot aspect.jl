...
begin
    CairoMakie.activate!()
    fig = Figure(size=(190*4,190*4.15+20));
    ...
    # megaplot -- less than
    ax1 = Axis(scattergrid[1,1], xticks = collect(0:10:maximum(df_all.steel_ec)), title = "a) Full Dataset", aspect = DataAspect(), xlabel = "EC steel kgCO2e/m²", ylabel = "EC RC-slab kgCO2e/m²", limits=(0,125,0,70), yticklabelsize = fontsize, xticklabelsize = fontsize, xlabelsize = fontsize, ylabelsize = fontsize, titlesize = fontsize);

    x_lim_min = minimum(less_than_bau.steel_ec)
    y_lim_min = minimum(less_than_bau.slab_ec)

    ax2 = Axis(gradient_grid[1,1], yticks = collect(0:10:bau_slab), aspect=DataAspect(), title = "b) Element density (inset)", xlabel = "EC steel kgCO2e/m²", ylabel = "EC RC-slab kgCO2e/m²", yticklabelsize = fontsize, xticklabelsize = fontsize, xlabelsize = fontsize, ylabelsize = fontsize, titlesize=fontsize,limits=(0,bau_steel * 1.1,0,bau_slab*1.1));
    ...
    axislegend(ax2, [elem_g, elem_s, elem_grey], ["Grid", "Star", "Non-standard"], position = :rb, orientation = :vertical, labelhalign = :right, framevisible = true, backgroundcolor= :white, framecolor = :white, labelsize=9, patchsize = (2,10), padding=(2,2,2,2))
    ...
    scatter!(ax1, df_star.steel_ec, df_star.slab_ec, marker=df_star.symbol, rotation=df_star.rotation, color=(:navy,alpha_fade), transparency = false, markersize=markersize, inspector_label = (self, i, p) -> df_star.rowcol[i])
    ...
    scatter!(ax1, df_star.steel_ec, df_star.slab_ec, marker=df_star.symbol, rotation=df_star.rotation, color=(:navy,alpha), transparency = transparency, markersize=markersize, inspector_label = (self, i, p) -> df_star.rowcol[i])
    ...
    # display
    display(fig)
    save("figures/megaplot_aspect.pdf", fig)
