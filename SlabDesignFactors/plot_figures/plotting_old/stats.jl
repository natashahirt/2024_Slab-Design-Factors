...
df_all = csv_2_df(["3_grasshopper_slabs_topology/", "3_grasshopper_slabs_grid/", "3_grasshopper_slabs_star/"], categories=["t","g","s"])
...
filter_category = row -> row.category == "s"
...
filter(filter_bau,df_all).total_ec[1] # business as usual
(filter(filter_bau,df_all).total_ec[1] - minimum(filter(filter_conventional, df_all).total_ec)) / filter(filter_bau,df_all).total_ec[1] # reduction for conventional
(filter(filter_bau,df_all).total_ec[1] - minimum(df_all.total_ec)) / filter(filter_bau,df_all).total_ec[1] # reduction for all 

println(filter(row -> row.total_ec == minimum(df_below_bau.total_ec), df_below_bau).total_ec)
...
begin
    ...
    for (k, df_plot) in enumerate([df_all, df_below_bau])
        ...
        ax1 = Axis(grid[1,1], title = "Total", ylabel = "EC kgCO2e/m²", xticks = (1:5, ["Slab sizing", "Beam\nsizing", "Beam\ncollinearity", "Assembly\ndepth", "Slab types"]), limits = (nothing,nothing,0,nothing), titlesize = fontsize, yticklabelsize = fontsize, xticklabelsize = fontsize, xlabelsize = fontsize, ylabelsize = fontsize)
    ...
    display(fig)
    save("figures/stats summary.pdf", fig)
...
begin
    ...
    colsize!(master_grid,1,Relative(1/2))
    texts = ["a) Full dataset", "b) Total EC < business-as-usual"]
    names = unique(filter(row -> row.category == "t", df_below_bau).rowcol)

    for (k, df_considered) in enumerate([df_all, df_below_bau])

        grid = grids[k]
        Label(grid[0, :], text = texts[k], fontsize = fontsize, font = :bold, tellwidth = false)
        df_plot = filter(row -> row.category == "t", df_considered)
        
        ax1 = Axis(grid[1,1], title = "Total", ylabel = "EC kgCO2e/m²", xticks = (1:lastindex(names), names), limits = (nothing,nothing,0,nothing), titlesize = fontsize, yticklabelsize = fontsize, xticklabelsize = smallfontsize, xlabelsize = fontsize, ylabelsize = fontsize, xticklabelrotation=pi/2)
        ax2 = Axis(grid[2,1], title = "Steel", ylabel = "EC kgCO2e/m²", xticks = (1:lastindex(names), names), limits = (nothing,nothing,0,nothing), titlesize = fontsize, yticklabelsize = fontsize, xticklabelsize = smallfontsize, xlabelsize = fontsize, ylabelsize = fontsize, xticklabelrotation=pi/2)
        ax3 = Axis(grid[3,1], title = "Slab", ylabel = "EC kgCO2e/m²", xticks = (1:lastindex(names), names), limits = (nothing,nothing,0,nothing), titlesize = fontsize, yticklabelsize = fontsize, xticklabelsize = smallfontsize, xlabelsize = fontsize, ylabelsize = fontsize, xticklabelrotation=pi/2)
    ...
    display(fig)
    save("figures/stats topology.pdf", fig)
