...
"""
function plot_5_beam_sizes(df_all::DataFrame; category=nothing)
    ...
    for max_depth in [25, 40]
        ...
        df_names = df_names[1:top_50_percent_index, :]
    ...
    ax = Axis(fig[1, 1], 
        title="b) Beam sizing by section", 
    ...
    for beam_sizer in ["discrete", "continuous"]
        ...
        for W_id in W_ids
            if occursin("W", W_id)
                split_name = split(W_id, r"(?<=\d)(?=\D)|(?<=\D)(?=\d)")
