...
df_unfixed = assemble_data("SlabDesignFactors/results/processed_results/fix_params.csv")
df_fixed = filter(row -> row.category == "topology", df_all)
...
fig = plot_11_geometries(df_all, category="grid")
save(save_path * "11_geometries_grid.pdf", fig)
fig = plot_11_geometries(df_all, category="nova")
save(save_path * "11_geometries_nova.pdf", fig)
...
df = CSV.read("SlabDesignFactors/results/processed_results/fix_params.csv", DataFrame)

for i in 1:nrow(df)
    sections = parse_sections(String(df[i, :sections])) 
    ids = parse_sections(String(df[i, :ids]))
    if sections != ids
        println(df[i, :name], "...", df[i, :collinear])
        df[i, :sections] = "Any[" * join(map(x -> "\"$x\"", ids), ", ") * "]"
    end
end

# Write back to CSV if needed
CSV.write("SlabDesignFactors/results/processed_results/fix_params.csv", df)

for row in CSV.Rows("SlabDesignFactors/results/processed_results/fix_params.csv")
